...
function render(activityOrders) {
  ...
  for(let activityOrder of activityOrders){
    ...
    <tr data-status="${data_status}">
      ...
      <td>${stateHtml}</td>
      <td>
        <a href="/Jamigo/activity/order/controll/activity_orderEdit.html?activityOrderNo=${activityOrder.activityOrderNo}" class="btn btn-edit">
...
document.querySelector('.panel-table').addEventListener('click', e => {
  if(e.target.matches('.btn-info')) {
    ...
    `;
    document.querySelector('.modal-footer a').href = `/Jamigo/activity/order/controll/activity_orderEdit.html?activityOrderNo=${activityOrder.activityOrderNo}`;
